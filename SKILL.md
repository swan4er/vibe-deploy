...
**ВАЖНО:** Все systemd-сервисы из `templates/systemd/` используют `User=app`. Приложение работает от непривилегированного пользователя. Файлы в `/opt/app` должны принадлежать `app:app`.
